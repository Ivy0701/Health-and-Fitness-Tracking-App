/**
 * Unified MET-based calories OUT (exercise burn) for the whole app.
 * Formula: round(MET × weightKg × durationMinutes / 60)
 * Keep in sync with frontend/src/utils/workoutCaloriesBurn.js
 */

const DEFAULT_WEIGHT_KG = 60;
const DEFAULT_MET = 4.0; // workout_default

/** Canonical MET table (snake_case keys + common synonyms). */
const MET_BY_NORMALIZED_KEY = {
  ...
  // Sports / legacy UI labels (reasonable MET approximations)
  basketball: 8.0,
  football: 8.0,
  badminton: 7.0,
  tennis: 7.0,
  // Legacy keys used in older seed / UI
  walk: 3.5,
  run: 8.0,
  swim: 6.0,
  rope: 12.3,
...
/**
 * @returns {{ met: number, usedDefault: boolean }}
 */
function resolveMetWithSource(category, title) {
...
function resolveWeightKg(weightKg) {
  const w = Number(weightKg);
  if (Number.isFinite(w) && w > 0) return w;
  return DEFAULT_WEIGHT_KG;
}

function exerciseEffectiveDurationMinutes(exercise) {
 ...
 * @returns {number} integer kcal
 */
function calculateWorkoutCaloriesBurned(p) {
 ...
 */
function applyMetBurnsToExercises(exercises, courseCategory, weightKg) {
  const w = resolveWeightKg(weightKg);
  const cat = String(courseCategory || "");
  return (Array.isArray(exercises) ? exercises : []).map((ex) => {
    const plain = typeof ex?.toObject === "function" ? ex.toObject() : { ...ex };
    const burn = calculateWorkoutCaloriesBurned({
      durationMinutes: exerciseEffectiveDurationMinutes(plain),
      category: cat,
      title: plain.title,
      weightKg: w,
    });
    return { ...plain, estimated_burn: burn };
  });
}
